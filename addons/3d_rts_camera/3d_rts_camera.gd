...
var orbit_distance: float = 10.0     # zoom applies to this
...
func _ready() -> void:
	...
	defaultYaw = _yaw
	defaultPitch = _pitch
	defaultOrbCent = orbit_center
	defaultOrbDis = orbit_distance
...
#	var speed_multiplier := 2.0 if Input.is_action_pressed("ui_shift") else 1.0
	...
	if Input.is_action_pressed("camera_load"):
		_yaw = defaultYaw
		_pitch = defaultPitch
		orbit_center = defaultOrbCent
		orbit_distance = defaultOrbDis
		_update_camera_position()
	...
	if Input.is_action_pressed("camera_save"):
		defaultYaw = _yaw
		defaultPitch = _pitch
		defaultOrbCent = orbit_center
		defaultOrbDis = orbit_distance
